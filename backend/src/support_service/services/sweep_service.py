"""Cron passes: SLA flagging, expired sessions, pruning, and media fetch (§7, §11).

Hourly sweep (`run_sweep`) and the 2-minute media pass (`run_media_pass`) are
both cron-triggered background jobs, kept together since they share the same
"scan a collection, fix up stragglers" shape.
"""
...
from support_service.models import EventType, Language, Session, SlaState
from support_service.repositories.base import from_firestore, get_db, to_firestore
from support_service.repositories.contact_repository import (
    contacts_with_recently_closed,
    expired_sessions,
    idle_description_sessions,
)
from support_service.repositories.ticket_repository import open_tickets_for_sla_sweep
from support_service.services.channel_service import send_text
from support_service.services.media_service import fetch_pending_media
...
def _flag_sla() -> int:
    ...
    open_tickets = open_tickets_for_sla_sweep(db, limit=100).get()
...
def _clear_expired_sessions() -> int:
    ...
    expired = expired_sessions(db, cutoff, limit=50).get()
...
def _prune_recently_closed() -> int:
    ...
    contacts = contacts_with_recently_closed(db, limit=50).get()
...
def run_media_pass() -> dict[str, int]:
    """Fetch pending media and finish idle description sessions (§7, §11)."""
...
def _finish_idle_sessions() -> int:
    ...
    idle = idle_description_sessions(db, cutoff, limit=20).get()
